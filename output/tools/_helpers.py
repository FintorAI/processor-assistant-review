...
def _efolder_present(state: dict, doc_type: str) -> bool:
    ...
    if isinstance(entry, dict):
        # Ground-truth presence: the number of attachments in the actual eFolder bucket,
        # taken from the GET /documents listing (not the extraction cache). This is the
        # authoritative signal — a document can physically exist (count > 0) while field
        # extraction failed (status=not_found), and a cached not_found record can carry
        # copy_count=1 for a bucket that does not actually exist. When this key is present
        # it is decisive; older states without it fall back to the copy_count heuristic.
