...
| **03 #12** — USPS multi-doc address cross-ref | ✅ **SHIPPED** — `review_borrower_summary.py` §3.12 cross-references the subject address against the USPS-validated address across **Tax Certificate** (`tax_property_address`), **Evidence of Insurance** (`insured_location` / `hazard_insurance_address`), and **Title Report** (`property_address`, source-filtered via `_sourced_property_address`) — on top of the pre-existing Purchase Contract + Appraisal checks. Mismatch → warning, match → info; never auto-corrected. The **Flood Certificate** address leg (12 #2) moved to the dedicated **STEP_08** `review_flood_hazard_insurance.py`. |
| **12 #2** — Applicant/property vs flood cert | ✅ **SHIPPED** — Migrated into the dedicated **STEP_08 Flood & Hazard Insurance** step (`review_flood_hazard_insurance.py`, substep 8.1; was `review_borrower_summary` §3.12/§3.12b). The live Flood Certificate catchingDoc schema already extracts `property_address` **and** `borrower_name` (plus `in_sfha`, `county`); registered in `required_docs.json`. The tool (a) cross-refs the flood-cert `property_address` vs the USPS-validated subject and (b) confirms the flood-cert `borrower_name` overlaps the Encompass applicant surname(s). Mismatch → warning, match → info; never auto-corrected. **No schema push needed.** |
| **12 #4** — Confirm Flood Zone Designation | ✅ **SHIPPED (auto-correct)** — Migrated into the dedicated **STEP_08** `review_flood_hazard_insurance.py` (substep 8.1; was `review_borrower_summary` §3.12c). Consumes the flood-cert `flood_zone` + `in_sfha`: classifies SFHA (A/V) vs non-hazard (X); when in an SFHA, warns if no flood policy is on file (Flood Insurance doc / `flood_policy_number` / `flood_annual_premium`); and compares the extracted zone against the **Encompass Flood Zone on the Flood Information form (field 541)**. **The flood determination is authoritative: on mismatch OR when 541 is blank, field 541 is auto-written from the cert** — but only when the extracted zone maps to a value the field-541 dropdown accepts (`_valid_flood_zone` → exact enum: `A, A99, AE, AH, AO, AR, AR/A, AR/AE, AR/AH, AR/AO, B, C, D, V, VE, V0, X, X500, XS, XU`, with numbered A/V zones collapsing to `A1-A30` / `V1-V30`). A non-standard OCR value is never written and keeps a warning (§3.13 policy). Writes emit an `info-overwrite` audit flag (no warning). Fields `541` (`los_flood_zone`) + `2363` (`los_flood_cert_number`, Cert #) registered in `FIELD_MAP` + step-08 YAML (factory-reset PASSED). |
...
| 03 URLA / 1003 | 12 | USPS Address Verification multi-doc cross-ref | ✅ (Tax/HOI/Title; flood leg → STEP_08) |
