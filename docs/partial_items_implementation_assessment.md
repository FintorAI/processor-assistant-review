...
| **12 #4** — Confirm Flood Zone Designation | ✅ **SHIPPED** — `review_borrower_summary.py` §3.12c consumes the flood-cert `flood_zone` + `in_sfha`: classifies SFHA (A/V) vs non-hazard (X); when in an SFHA, warns if no flood policy is on file (Flood Insurance doc / `flood_policy_number` / `flood_annual_premium`); and compares the extracted zone against the **Encompass Flood Zone (field 1846)** — mismatch → warning, agree → info, LOS blank while doc has a zone → warning. Field `1846` (key `los_flood_zone`) registered in `FIELD_MAP` + step-02 YAML (factory-reset PASSED). LOS zone is never auto-corrected. |
