...
The event names and payload shapes must match what the LangGraph SDK / stdlib
clients expect (`event: metadata` with run_id, per-mode token events,
`event: end`), otherwise clients silently drop tokens or fail to parse.
"""
